...
import praeterii.pixels.artlist.ArtListActivity;
import praeterii.pixels.shapes.Bucket;
import praeterii.pixels.shapes.Circle;
import praeterii.pixels.shapes.Line;
import praeterii.pixels.shapes.Pen;
import praeterii.pixels.shapes.Pencil;
import praeterii.pixels.shapes.Rectangle;
import praeterii.pixels.shapes.Shape;
import praeterii.pixels.shapes.ShapeEditor.ShapeFactory;
...
public class PixelArtEditor extends PApplet implements TouchListener, Drawer {
	...
	public void onCreate(final Bundle savedinstance) {
		...
		bbbar = (RelativeLayout) getLayoutInflater().inflate(praeterii.pixele.R.layout.buttonbar, null);
		this.setContentView(bbbar,
				new RelativeLayout.LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT));
		buttonbar = bbbar.findViewById(praeterii.pixele.R.id.buttonbarz);
		ViewGroup g = bbbar.findViewById(praeterii.pixele.R.id.surfaceholder);
	...
	private class ScaleListener extends ScaleGestureDetector.SimpleOnScaleGestureListener {
		...
		public boolean onScale(ScaleGestureDetector detector) {
			// if (state.mode == PixelArtState.DRAW || state.mode ==
			// PixelArtState.ERASER) return false;
			float[] coords = art.getDataCoordsFloatFromXY(PixelArtEditor.this, detector.getFocusX(),
					detector.getFocusY());
			if (passedthresh || Math.abs(origdist - detector.getCurrentSpan()) > thresh) {
				...
				// Log.d("Pixelesque", "SCALE: moving: "+diffx+", "+diffy + "
				// orig:"+coords[0]+","+coords[1]+ " post:
	...
	public boolean surfaceTouchEvent(final MotionEvent event) {
		if (mtManager != null)
			mtManager.surfaceTouchEvent(event);
	...
	public void touchDown(Cursor c) {
		...
		if (mScaleDetector != null && !mScaleDetector.isInProgress()
				&& art.isValid(art.getDataCoordsFromXY(this, c.firstPoint.x, c.firstPoint.y))) {
			if (DEBUG)
	...
	public void touchMoved(Cursor c) {
		...
		if (DEBUG)
			Log.d("Pixelesque", "MOV: c1:(" + coords1[0] + "," + coords1[1] + ")  c2:(" + coords2[0] + "," + coords2[1]
					+ ")   movcur:" + movingCursor);
		...
		if (state.mode == PixelArtState.PENCIL) {
			if (art.shapeeditor.hasCursor(c)) {
				if (coords1[0] == coords2[0] && coords1[1] == coords2[1] && !mScaleDetector.isInProgress()) {
					...
					if (DEBUG)
						Log.d("Pixelesque", "MOV REMOVE: c1:(" + coords1[0] + "," + coords1[1] + ")  c2:(" + coords2[0]
								+ "," + coords2[1] + ")   movcur:" + movingCursor);
	...
	public void touchUp(Cursor c) {
		...
		if (DEBUG)
			Log.d("Pixelesque",
					"UP: c1:(" + coords1[0] + "," + coords1[1] + ")  c2:(" + coords2[0] + "," + coords2[1] + ")");
	...
	void actualsetup() {
		...
		buttonbar.setParent(findViewById(praeterii.pixele.R.id.layout));
	...
	class OpenArtTask extends AsyncTask<String, Void, PixelArt> {
		...
		protected void onPreExecute() {
			String title = getResources().getString(praeterii.pixele.R.string.loading_title);
			String text = getResources().getString(praeterii.pixele.R.string.loading_text);
		...
		protected PixelArt doInBackground(String... params) {
			...
			try {
				ArtExtras.populateExtras(PixelArtEditor.this, PixelArtEditor.this, art,
						new File(path).getName().replace(".png", ""));
			} catch (Exception ee) {
				ee.printStackTrace();
				runOnUiThread(new Runnable() {
					public void run() {
						Toast.makeText(PixelArtEditor.this, praeterii.pixele.R.string.open_extras_failed,
		...
		protected void onPostExecute(PixelArt result) {
			...
			if (result != null) {
				...
				Toast.makeText(PixelArtEditor.this, praeterii.pixele.R.string.open_failed, Toast.LENGTH_SHORT).show();
			d.dismiss();
			scheduleRedraw();
		}
	}
	...
	public void checkBounds() {
		...
		if (art.topy > 0)
			art.topy = 0;
		if (art.topx + art.getWidth(this) < this.width)
			art.topx = Math.min(0, (int) (width - art.getWidth(this)));
		if (art.topy + art.getHeight(this) < this.height)
			art.topy = Math.min(0, (int) (height - art.getHeight(this)));

		if (art.getWidth(this) < this.width)
			art.topx = (this.width - art.getWidth(this)) / 2;
	}
	...
	public class ChangeName implements Runnable {
		public void run() {
			TextView view = (TextView) findViewById(praeterii.pixele.R.id.picturename);
			if (art != null && art.name != null) {
				if (view != null)
					view.setText(art.name);
				// setTitle("Pixelesque - "+art.name);
				setTitle(art.name);
			} else {
				String title = getResources().getString(praeterii.pixele.R.string.new_art_title);
				if (view != null)
					view.setText(title);
	...
	public boolean onCreateOptionsMenu(final Menu menu) {
		...
		inflater.inflate(praeterii.pixele.R.menu.mainmenu, menu);
		return true;
	}
	...
	public boolean onMenuItemSelected(final int featureId, final MenuItem item) {
		...
		case praeterii.pixele.R.id.main_menu_save:
			save();
			return true;
		case praeterii.pixele.R.id.main_menu_save_as:
			saveas();
			return true;
		case praeterii.pixele.R.id.main_menu_export:
			export();
			return true;
		case praeterii.pixele.R.id.main_menu_share:
			share();
			return true;
		case praeterii.pixele.R.id.main_menu_open:
			load();
			return true;
		case praeterii.pixele.R.id.main_menu_new:
			shownew();
			return true;
		case praeterii.pixele.R.id.main_menu_clear:
			clear();
			return true;
		case praeterii.pixele.R.id.main_menu_gridtoggle:
			toggleGrid();
			return true;
		// case praeterii.pixele.R.id.main_menu_background:
		// importBackground();
		// return true;
		case praeterii.pixele.R.id.main_menu_about:
			about();
			return true;
		...
		if (requestCode == LOAD_ACTIVITY) {
			...
			if (resultCode == RESULT_OK) {
				try {
					...
					Toast.makeText(this, praeterii.pixele.R.string.custombg_failed, Toast.LENGTH_LONG).show();
				}
			}
	...
	public void export(int longside, String extra) {
		...
		if (longside == EXPORT_ORIGINAL) {
			...
			if (art.name != null)
				export(art.name + extra, art.width, art.height);
			else
				export(null, art.width, art.height);
		}
		if (art.width > art.height) {
			if (art.name != null)
				export(art.name + extra, longside, -1);
			else
				export(null, longside, -1);
		} else {
			if (art.name != null)
				export(art.name + extra, -1, longside);
			else
				export(null, -1, longside);
		}
	}
	...
	public void share(int longside) {
		if (art.width > art.height) {
			if (art.name != null)
				share(art.name + "shared", longside, -1);
			else
				share(null, longside, -1);
		} else {
			if (art.name != null)
				share(art.name + "shared", -1, longside);
			else
				share(null, -1, longside);
		}
	}
